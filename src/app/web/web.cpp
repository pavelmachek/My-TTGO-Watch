...
static void enter_web_widget_event_cb( lv_obj_t * obj, lv_event_t event );

/*
 * setup routine for web app
 */
void web_setup( void ) {
    ...
    web_main_tile_num = mainbar_add_app_tile( 1, 1, "web" );
    web_setup_tile_num = mainbar_add_setup_tile( 1, 1, "web setup" );
    ...
    // use https://lvgl.io/tools/imageconverter to convert your images and set "true color with alpha" to get fancy images
    // the resulting c-file can put in /app/webs/images/ and declare it like LV_IMG_DECLARE( your_icon );
    web_widget = widget_register( "myapp", &web_64px, enter_web_widget_event_cb );
    widget_set_indicator( web_widget, ICON_INDICATOR_UPDATE );
#endif // WEB_WIDGET
...
static void enter_web_widget_event_cb( lv_obj_t * obj, lv_event_t event ) {
    switch( event ) {
        case( LV_EVENT_CLICKED ):       statusbar_hide( true );
                                        widget_hide_indicator( web_widget );
